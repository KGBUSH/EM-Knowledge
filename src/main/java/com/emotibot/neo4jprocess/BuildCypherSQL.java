...
public class BuildCypherSQL implements CypherSQLParser {
    ...
    {
    	if(pageInfo==null) return Common.EMPTY;
	...
	public String FindEntityInfo(String label, String name) {
		...
		query = "match (e:" + label + "{"+Common.KGNODE_NAMEATRR+":\"" + name + "\"}) return e." + Common.KG_NODE_FIRST_PARAM_ATTRIBUTENAME + " as "+Common.ResultObj;
		}
	...
	public String FindEntityAttr(String label, String name, String attr) {
		...
		if(Tool.isStrEmptyOrNull(label)||Tool.isStrEmptyOrNull(name)){}
		{
		  query = "match (e:" + label + "{"+Common.KGNODE_NAMEATRR+":\"" + name + "\"}) return e." + attr + " as "+Common.ResultObj;
		}
